    ...
    def initialize_system(self):
        ...
        try:
            ...
            self.graph_rag_retrieval = GraphRAGRetrieval(
                config=self.config, llm_client=self.generation_module.client
            )
        ...
        try:
            ...
            if relevant_docs:
                ...
                for doc in relevant_docs:
                    ...
                    search_type = doc.metadata.get(
                        "search_type", doc.metadata.get("route_strategy", "unknown")
                    )
                ...
                print(
                    f"📋 找到 {len(relevant_docs)} 个相关文档: {', '.join(doc_info[:3])}"
                )
                if len(doc_info) > 3:
            ...
            if stream:
                try:
                    for (
                        chunk_text
                    ) in self.generation_module.generate_adaptive_answer_stream(
